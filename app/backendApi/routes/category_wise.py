...
#     if metric.lower() in ['avg_ratings']:
#         return to_native(df['ratings'].mean())
...
#     elif metric == 'Average ratings':
#         return (data['average_ratings'].mean()) 
#     elif metric == 'Search visibility share Organic':
#         return (data['search_visibility_share___organic_'].mean())  
#     elif metric == 'Search visibility share Paid':
#         return (data['search_visibility_share___paid_'].mean())        

    
#     elif metric == 'Amazon Best seller rank':

#         return (data['amazon_best_seller_rank'].mean()) 
    
#     elif metric == 'Net sentiment of reviews':

#         return (data['net_sentiment_of_reviews'].mean()) 
#     elif metric == 'Net sentiment':

#         return (data['net_sentiment'].mean()) 
#     elif metric == 'Availability%':

#         return (data['availability_'].mean()) 
...
#                 if combined_data is not None and not combined_data.empty:
#                     raw_values = []
#                     if metric in ['Amazon Best seller rank', 'amazon_best_seller_rank', 'Availability%', 'availability_', 'Average ratings', 'average_ratings', 'Content score', 'content_score', 'Net sentiment', 'Net sentiment of reviews', 'net_sentiment_of_reviews', 'Organic Rank', 'organic_rank', 'Search visibility share Organic', 'search_visibility_share___organic_', 'Search visibility share (Paid)', 'search_visibility_share___paid_','Availability%','Average ratings','Net sentiment of reviews','Amazon Best seller rank','CTR', 'Engagement %', 'Transaction Rate', 'Cost per Transaction', 'Product Views per Session', 'Sessions to Product Views %', 'Product Views to Cart %']:
#                         # Handle special metrics by calculating for each brand and aggregating
#                         for brand in brands:
#                             brand_data = combined_data[combined_data['brand'] == brand]
#                             if not brand_data.empty:
...
def calculate_special_metrics(data, metric):
    if metric == 'CTR':
        ...
        return (data['average_ratings'].mean()) 
    elif metric == 'Mobile page speed insights score':
        return (data['mobile_page_speed_insights_score'].mean()) 
    elif metric == 'Web page speed insights score':
        return (data['web_page_speed_insights_score'].mean()) 
    elif metric == 'Largest contentful paint (LCP) - seconds':
        return (data['largest_contentful_paint__lcp____seconds'].mean()) 
    
    elif metric == 'Organic rank':
        return (data['organic_rank'].mean()) 
    elif metric == 'Search visibility share Organic':
        return (data['search_visibility_share___organic_'].mean())  
    elif metric == 'Search visibility share Paid':
        return (data['search_visibility_share___paid_'].mean())        
    ...
    for platform in platforms:
        for metric in metrics:
            ...
            if analysis_type == 'Overall':
                ...
                if combined_data is not None and not combined_data.empty:
                    raw_values = []
                    if metric in ['Mobile page speed insights score','Web page speed insights score','Largest contentful paint (LCP) - seconds','Organic rank','Amazon Best seller rank', 'amazon_best_seller_rank', 'Availability%', 'availability_', 'Average ratings', 'average_ratings', 'Content score', 'content_score', 'Net sentiment', 'Net sentiment of reviews', 'net_sentiment_of_reviews', 'Organic Rank', 'organic_rank', 'Search visibility share Organic', 'search_visibility_share___organic_', 'Search visibility share (Paid)', 'search_visibility_share___paid_','Availability%','Average ratings','Net sentiment of reviews','Amazon Best seller rank','CTR', 'Engagement %', 'Transaction Rate', 'Cost per Transaction', 'Product Views per Session', 'Sessions to Product Views %', 'Product Views to Cart %']:
                        # Handle special metrics by calculating for each brand and aggregating
                        for brand in brands:
                            brand_data = combined_data[combined_data['brand'] == brand]
                            if not brand_data.empty:
